...
describe('CounterpointValidator - getBrokenRules', () => {
  ...
  it('flags dissonant interval as Error', () => {
    ...
    const brokenRules = validator.getBrokenRules(cf, cp);
    expect(brokenRules.map(r => r.description)).toContain('No harmonic dissonances (fourths are dissonant in two-voice exercises but constant in three-voice exercise between upper voices)');
    expect(brokenRules.find(r => r.description === 'No harmonic dissonances (fourths are dissonant in two-voice exercises but constant in three-voice exercise between upper voices)')?.severity).toBe(Severity.Error);
  });
  ...
  it('flags final cadence not approached by step as Warning', () => {
    ...
    const brokenRules = validator.getBrokenRules(dorianCF, cp);
    expect(brokenRules.map(r => r.description)).toContain('Approach the final note by step');
    expect(brokenRules.find(r => r.description === 'Approach the final note by step')?.severity).toBe(Severity.Warning);
  });
  ...
  it('flags parallel perfect fifths as Error', () => {
    ...
    const brokenRules = validator.getBrokenRules(cf, cp);
    expect(brokenRules.map(r => r.description)).toContain('No parallel perfect fifths');
    expect(brokenRules.find(r => r.description === 'No parallel perfect fifths')?.severity).toBe(Severity.Error);
  });
  ...
  it('flags parallel octaves as Error', () => {
    ...
    const brokenRules = validator.getBrokenRules(cf, cp);
    expect(brokenRules.map(r => r.description)).toContain('No parallel octaves');
    expect(brokenRules.find(r => r.description === 'No parallel octaves')?.severity).toBe(Severity.Error);
  });
  ...
  it('flags parallel unisons as Error', () => {
    ...
    const brokenRules = validator.getBrokenRules(cf, cp);
    expect(brokenRules.map(r => r.description)).toContain('No parallel unisons');
    expect(brokenRules.find(r => r.description === 'No parallel unisons')?.severity).toBe(Severity.Error);
  });
  ...
  it('flags unison in a middle position as Error', () => {
    ...
    const brokenRules = validator.getBrokenRules(cf, cp);
    expect(brokenRules.map(r => r.description)).toContain('Unisons are only permitted at the first and last note');
    expect(brokenRules.find(r => r.description === 'Unisons are only permitted at the first and last note')?.severity).toBe(Severity.Error);
  });
  ...
  it('flags hidden perfect interval as Error', () => {
    ...
    const brokenRules = validator.getBrokenRules(cf, cp);
    expect(brokenRules.map(r => r.description)).toContain('No approaching a perfect consonance by direct motion');
    expect(brokenRules.find(r => r.description === 'No approaching a perfect consonance by direct motion')?.severity).toBe(Severity.Error);
  });
  ...
  it('flags dissonant melodic leap (tritone) as Error', () => {
    ...
    const brokenRules = validator.getBrokenRules(dorianCF, cp);
    expect(brokenRules.map(r => r.description)).toContain('No dissonant melodic leaps (sevenths, tritone, or larger than octave)');
    expect(brokenRules.find(r => r.description === 'No dissonant melodic leaps (sevenths, tritone, or larger than octave)')?.severity).toBe(Severity.Error);
  });
  ...
  it('flags large leap not recovered as Warning', () => {
    ...
    expect(brokenRules.find(r => r.description === 'Leaps larger than a third should be recovered by a step in the opposite direction')?.severity).toBe(Severity.Warning);
  });
  ...
  it('flags more than 3 consecutive thirds as Warning', () => {
    ...
    const brokenRules = validator.getBrokenRules(cf, cp);
    expect(brokenRules.map(r => r.description)).toContain('Avoid more than 3 consecutive thirds or sixths');
    expect(brokenRules.find(r => r.description === 'Avoid more than 3 consecutive thirds or sixths')?.severity).toBe(Severity.Warning);
  });
  ...
  it('flags overused tone repetition as Warning', () => {
    ...
    const brokenRules = validator.getBrokenRules(dorianCF, cp);
    expect(brokenRules.map(r => r.description)).toContain('Avoid overusing tone repetition');
    expect(brokenRules.find(r => r.description === 'Avoid overusing tone repetition')?.severity).toBe(Severity.Warning);
  });
